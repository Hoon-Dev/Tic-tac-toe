...
describe("Tic-tac-toe", async () => {
  ...
  const userOneKeypair = 
    USE_STATIC_WALLET ? anchor.web3.Keypair.fromSecretKey(new Uint8Array([
  ...
  it("User1 create room with bet 100 lamport", async () => {
    ...
    let userOneInfo = await fetchAccount(program, "playerInfo", userOneInfoPDA, "user1");

    if(userOneInfo == undefined) {
    ...
    await fetchAccount(program, "room", userOneRoomPDA, "user1 room");
  });
